...
def render(
    ...
    with st.expander("¿Cómo se calcula esta cartera?"):
        st.markdown(
            f"1. **Rentabilidad esperada de cada activo (CAPM)**: se calcula a partir de la tasa "
            f"libre de riesgo (Rf = {format_percentage(config.RISK_FREE_RATE, decimals=1)}), la "
            f"rentabilidad de mercado (Rm = {format_percentage(config.MARKET_RETURN, decimals=1)}) "
            "y la Beta histórica propia de cada activo (ver Hoja 3). Esto da, para cada activo, la "
            "rentabilidad que cabría esperar de él según su riesgo.\n"
            "2. **Cómo optimiza Markowitz**: con esas rentabilidades esperadas y con la volatilidad y "
            "las correlaciones históricas entre todos los activos, se calculan todas las "
            "combinaciones de pesos posibles que respetan las restricciones de tu perfil (ver punto "
            "4). Para cada combinación se obtiene su rentabilidad y su riesgo conjuntos — que no son "
            "solo la suma de los riesgos individuales, sino que dependen de cómo se mueven los "
            "activos entre sí (la diversificación reduce el riesgo total).\n"
            "3. **Cómo elige el Ratio de Sharpe la mejor combinación**: de entre todas esas "
            "combinaciones posibles, se selecciona la que ofrece más rentabilidad por cada unidad de "
            "riesgo asumido — es decir, el mayor Ratio de Sharpe. Esa es la cartera que ves a "
            "continuación.\n"
            f"4. **Por qué un activo recibe un peso concreto**: dentro de esa búsqueda, un activo con "
            "mejor relación rentabilidad/riesgo y baja correlación con los demás tiende a recibir más "
            f"peso, hasta el máximo permitido para tu perfil "
            ...
            "5. **Del peso al capital asignado**: el importe de cada posición es, simplemente, "
            "`peso × capital total invertido` — repartido en céntimos exactos para que la suma "
            "coincida siempre con tu importe (columna \"Capital asignado\" en la tabla de abajo)."
        )
